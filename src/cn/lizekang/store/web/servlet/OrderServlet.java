...
import cn.lizekang.store.service.serviceImp.OrderServiceImp;
import cn.lizekang.store.utils.PaymentUtil;
...
public class OrderServlet extends BaseServlet {
	...
	public String saveOrder(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//创建订单对象,为订单对象赋值        	
		Cart cart=(Cart) request.getSession().getAttribute("cart");//获取购物车（订单对象有些值需要）
		...
		for (CartItem item : cart.getCartItem()) {
			...
			orderItem.setOrder(order);//设置当前的订单项属于那个订单
			
			order.getList().add(orderItem);//将订单项加入订单里面，接下来的程序里从订单对象获取订单项的数据而不用再另外加入订单项
		}
		//调用业务层功能:保存订单(即是将订单和订单项存进数据库)
