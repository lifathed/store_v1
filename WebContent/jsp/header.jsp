...
<html>
	...
	<script >
		$(function(){
			//向服务端CategoryServlet-->getAllCats发起Ajax请求，服务的经过处理将所有分类信息以json格式返回，
			//将获取到返回的所有分类绑定在页面的显示分类区域
			var url="/store_v1/CategoryServlet";//请求地址
			var obj={"method":"findAllCats"};//需要传递到服务器端的参数。 参数形式为“键/值”
			$.post(url,obj,function(data){
				//alert(data);
				
			//获取到服务端响应回来的数据，经过观察data存放的是一个json格式数组，遍历数组，动态显示分类区域代码
			$.each(data,function(i,obj){ //i是下标
				var li="<li><a href='/store_v1/ProductServlet?method=findProductsByCidWithPage&num=1&cid="+obj.cid+"'>"+obj.cname+"</a></li>";
				$("#myUI").append(li);//指定显示到那个区域，myUI是分类显示区域的ID
			});			
			},"json");
		});
	</script>
